...
var TouchCarousel = function (_React$Component) {
  _inherits(TouchCarousel, _React$Component);
  ...
  function TouchCarousel(props) {
    ...
    _this.onTouchEndOrCancel = function (e) {
      ...
      if (wasDragging && _this.touchMoves.length) {
        ...
        var cards = -(_this.props.cardCount - _this.props.cardsOnPage);
        targetCursor = Math.round(cursor + cursorDelta) > 0 ? 0 : Math.round(cursor + cursorDelta);
        if (targetCursor < cards) {
          targetCursor = cards;
        }
        // targetCursor = Math.round(cursor + cursorDelta) < cards ? cards : Math.round(cursor + cursorDelta);
        _this.touchMoves = [];
      } else {
  ...
  _createClass(TouchCarousel, [{
    ...
    value: function getComputedCursor() {
      var _props = this.props,
        ...
        cardsOnPage = _props.cardsOnPage;
      ...
      if (!loop) {
        ...
        if (dragging && cursor < 1 - cardCount && cardsOnPage === 1) {
          computedCursor = 1 - cardCount - maxOverflow + maxOverflow / (1 - cardCount - cursor + 1)
        }
        if (dragging && computedCursor < cardsOnPage - cardCount) {
          computedCursor = cardsOnPage - cardCount - maxOverflow + maxOverflow / (cardsOnPage - cardCount - cursor + 1);;
        } else if (computedCursor < cardsOnPage - cardCount) {
          computedCursor = cardsOnPage - cardCount;
        }
      }
    ...
    value: function render() {
      ...
      var _props2 = this.props,
        ...
        loop = _props2.loop,
        cardsOnPage = _props2.cardsOnPage,
        rest = _objectWithoutProperties(_props2, ['component', 'cardSize', 'cardCount', 'cardPadCount', 'renderCard', 'stiffness', 'damping', 'precision', 'loop', 'cardsOnPage']);
